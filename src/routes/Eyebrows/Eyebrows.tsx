...
import { useEyebrowsQuery } from "../../redux/api/productsApi";
...
const Eyebrows: React.FC = () => {
  ...
  return (
    <div className="container mx-auto py-10 px-8 border border-gray-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
        {data.slice(1, 25).map((product: Tblush) => (
          <Card
            key={product.id}
            hoverable
            cover={
              <div className="h-40 flex justify-center items-center overflow-hidden">
                <img
                  ...
                  className="w-32 h-32 object-cover rounded-full transition-transform duration-300  ml-[100px] mt-[10px]" 
                  onError={(e) => {
