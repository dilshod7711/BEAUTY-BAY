...
import { useNailpolishesQuery } from "../../redux/api/productsApi";
...
const Nailpolishes: React.FC = () => {
  const dispatch = useDispatch();
  const { data, isLoading } = useNailpolishesQuery();
  ...
  return (
    <div className="container mx-auto py-10 px-8 border border-gray-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
        {data.slice(14).map((product: Tblush) => (
          <Card
            key={product.id}
            hoverable
            cover={
