...
import { useBronzerQuery } from "../../redux/api/productsApi";
...
const Bronzer: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto py-10 px-8 border border-gray-200">
        <Title level={2} className="text-center mb-10 mt-6">Bronzer Collection</Title>
        {renderSkeletons()}
      </div>
    );
  }
  ...
  if (!Array.isArray(data) || data.length === 0) {
    return (
      <Empty
        description="Hech qanday mahsulot topilmadi"
        className="my-16"
      />
    );
  }
  ...
  return (
    <div className="container mx-auto py-10 px-8 border border-gray-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
        {data.slice(21).map((product: Tblush) => (
          <Card
            key={product.id}
            hoverable
            cover={
              <div className="h-40 flex justify-center items-center p-4 ">
                <img
                  ...
                  className="w-32 h-32 object-cover rounded-full ml-[100px] transition-transform duration-300  flex  justify-center"
                  onError={(e) => {
